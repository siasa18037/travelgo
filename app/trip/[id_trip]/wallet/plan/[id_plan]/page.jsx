  ...
  return (
    <>
      <div className='WalletPage container'>
        <div className="button-link mt-3 d-flex align-items-center justify-content-between gap-4">
          <h4 className="d-flex align-items-center mb-0 gap-2">
            <Wallet size={24} className="" />
            Wallet
            <p className='mb-0 small'>({planItem?.name || ''})</p>
            
          </h4>
          <div className="d-flex align-items-center gap-2">
            <a className='btn input-outline-dark' href={`/trip/${id_trip}/plan/${id_plan}`}>
              Plan
            </a>
      ...
      {showCreateWallet && (
        <CreateWallet
          ...
          plan_price={planItem?.amount}
          notes={`${planItem?.name || ''} ราคารวม ${planItem?.amount?.price || ''} ${planItem?.amount?.currency || ''}`}
          perDescription={`จ่ายค่า ${planItem?.type} ${planItem?.name || ''} `}
        />
      )}
      {showShowDetailBox && (
          <ShowDetailBox
